...
<xsl:template match="select" mode="ixsl:onchange">
  ...
  <xsl:result-document href="#ingredients" method="ixsl:replace-content">
    <xsl:apply-templates select="//div[@id='ingredients']/*" mode="fix-servings">
      <xsl:with-param name="factor" tunnel="yes"
                      select="$servings div $base-servings"/>
    </xsl:apply-templates>
  </xsl:result-document>
</xsl:template>
...
<xsl:template match="li[@x-quantity]" mode="fix-servings">
  <xsl:param name="factor" as="xs:double" tunnel="yes"/>
  <xsl:copy>
    <xsl:apply-templates select="@*" mode="fix-servings"/>
